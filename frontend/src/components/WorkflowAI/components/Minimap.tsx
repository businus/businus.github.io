...
interface MinimapProps {
    ...
    onPan: (transform: { x: number; y: number; scale: number }) => void;
}
...
export const Minimap: React.FC<MinimapProps> = ({ nodes, edges, viewTransform, canvasDimensions, onPan }) => {
            ...
            >
                <g>
                    {nodes.map(node => {
                        ...
                        if (node.type === NodeType.TRIGGER) color = '#7c3aed'; // brand-secondary
                        if (node.type === NodeType.IF_CONDITION) color = '#db2777'; // pink
