...
import { WorkflowNode, Edge, NodeType, NodeGroup, ExecutionStatus } from '../types';
...
interface WorkflowCanvasProps {
  ...
  edges: Edge[];
  groups: NodeGroup[];
  onNodeClick: (nodeId: string, isShiftPressed: boolean) => void;
  onCanvasClick: () => void;
  onSelectNodes: (nodeIds: string[], isShiftPressed: boolean) => void;
...
const getEdgePath = (
...
): { path: string; label: string | null; midPoint: { x: number; y: number } } => {
  ...
  if (sourceNode.type === NodeType.IF_CONDITION) {
    sourcePos.y += sourceHandle === 'true' ? NODE_HEIGHT / 4 : NODE_HEIGHT * 3 / 4;
  } else if (sourceNode.type === NodeType.LOOP_START) {
...
export const WorkflowCanvas: React.FC<WorkflowCanvasProps> = ({
  ...
  const [draggingItem, setDraggingItem] = useState<{ type: 'node' | 'group', id: string, offset: { x: number, y: number } } | null>(null);
  const [drawingEdge, setDrawingEdge] = useState<{ sourceId: string; sourceHandle?: Edge['sourceHandle'], startPos: { x: number; y: number }, endPos: { x: number; y: number } } | null>(null);
  const [hoveredInputTargetId, setHoveredInputTargetId] = useState<string | null>(null);
  ...
  const handleStartDrawingEdge = useCallback((sourceId: string, sourceHandle?: Edge['sourceHandle'], startPosition?: {x: number, y: number}) => {
  ...
  const handleMouseUpOnNodeInput = useCallback((targetId: string) => {
    if (drawingEdge) {
  ...
  const renderedLoopFeedbackLines = useMemo(() => {
    const loopStarts = nodes.filter(n => n.type === NodeType.LOOP_START);
    const loopEnds = nodes.filter(n => n.type === NodeType.LOOP_END);
    const lines = [];
  ...
  return (
    <div
      ref={canvasRef}
      className="w-full h-full bg-dark-bg overflow-hidden relative"
    ...
    >
      ...
      {marqueeRect && (
          <div
              className="absolute border border-brand-primary bg-brand-primary/20 pointer-events-none"
              style={{
      ...
      />
      <div className="absolute bottom-4 right-4 bg-dark-surface p-1 rounded-md shadow-lg flex items-center gap-1 border border-dark-border z-10">
          <button onClick={onRun} disabled={executionStatus === ExecutionStatus.RUNNING} title="Run Workflow" className="p-1.5 hover:bg-dark-bg rounded-md text-dark-text-secondary hover:text-white disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-transparent"><RunIcon className="w-5 h-5"/></button>
          <div className="w-px h-6 bg-dark-border mx-1"></div>
          <button onClick={onUndo} disabled={!canUndo} title="Undo (Ctrl+Z)" className="p-1.5 hover:bg-dark-bg rounded-md text-dark-text-secondary hover:text-white disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-transparent"><UndoIcon className="w-5 h-5"/></button>
          <button onClick={onRedo} disabled={!canRedo} title="Redo (Ctrl+Y)" className="p-1.5 hover:bg-dark-bg rounded-md text-dark-text-secondary hover:text-white disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-transparent"><RedoIcon className="w-5 h-5"/></button>
          <div className="w-px h-6 bg-dark-border mx-1"></div>
          <button onClick={() => setViewTransform(v => ({...v, scale: Math.min(2, v.scale * 1.2)}))} className="p-1.5 hover:bg-dark-bg rounded-md text-dark-text-secondary hover:text-white"><ZoomInIcon className="w-5 h-5"/></button>
          <span className="text-xs font-mono text-dark-text-secondary w-10 text-center">{(viewTransform.scale * 100).toFixed(0)}%</span>
          <button onClick={() => setViewTransform(v => ({...v, scale: Math.max(0.2, v.scale / 1.2)}))} className="p-1.5 hover:bg-dark-bg rounded-md text-dark-text-secondary hover:text-white"><ZoomOutIcon className="w-5 h-5"/></button>
          <button onClick={() => setViewTransform({x:0, y:0, scale:1})} className="p-1.5 hover:bg-dark-bg rounded-md text-dark-text-secondary hover:text-white"><ResetZoomIcon className="w-5 h-5"/></button>
          <div className="w-px h-6 bg-dark-border mx-1"></div>
          <button onClick={onImport} title="Import Workflow" className="p-1.5 hover:bg-dark-bg rounded-md text-dark-text-secondary hover:text-white"><ImportIcon className="w-5 h-5"/></button>
          <button onClick={onExport} title="Export Workflow" className="p-1.5 hover:bg-dark-bg rounded-md text-dark-text-secondary hover:text-white"><ExportIcon className="w-5 h-5"/></button>
      </div>
    </div>
  );
};