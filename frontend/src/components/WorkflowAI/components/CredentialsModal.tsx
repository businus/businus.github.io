...
interface CredentialsModalProps {
    ...
    onAddCredential: (name: string, type: CredentialType, data: { [key: string]: string }) => void;
    onDeleteCredential: (id: string) => void;
}

const CredentialForm: React.FC<{ onAddCredential: CredentialsModalProps['onAddCredential'] }> = ({ onAddCredential }) => {
    const [name, setName] = useState('');
    const [type, setType] = useState<CredentialType>(CredentialType.API_KEY);
    const [data, setData] = useState<{ [key: string]: string }>({ apiKey: '' });

    const handleTypeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        const newType = e.target.value as CredentialType;
        setType(newType);
        if (newType === CredentialType.API_KEY) {
            setData({ apiKey: '' });
        } else if (newType === CredentialType.DATABASE) {
            ...
            <div>
             ...
             <div>
                ...
                <select value={type} onChange={handleTypeChange} className="w-full bg-dark-surface border border-dark-border rounded-md p-2 text-sm text-dark-text-primary focus:ring-1 focus:ring-brand-primary focus:outline-none transition">
                    ...
                    <option value={CredentialType.DATABASE}>Database</option>
                </select>
            </div>
...
export const CredentialsModal: React.FC<CredentialsModalProps> = ({ isOpen, onClose, credentials, onAddCredential, onDeleteCredential }) => {
