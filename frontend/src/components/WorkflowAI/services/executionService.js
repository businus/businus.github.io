import { NodeType, ExecutionStatus } from '../types.js';
...
export async function executeWorkflow({ nodes, edges, credentials, onLog, onSetActiveNode }) {
    const context = {};
    const nodeMap = new Map(nodes.map(n => [n.id, n]));

    const log = (nodeId, message, type = 'info') => {
    ...
    try {
        ...
        let currentNodeId = nodes.find(n => n.type === NodeType.TRIGGER)?.id || null;
        ...
        while (currentNodeId) {
            ...
            let output = null;
            let nextNodeId = null;
            let nextSourceHandle = undefined;
            ...
            switch (node.type) {
                ...
                case NodeType.CODE:
                    try {
                        ...
                        throw new Error(`Error in CODE node: ${e.message}`);
                    }
                    break;
