...
export async function generateWorkflowFromPrompt(prompt: string): Promise<{ type: string, label: string }[]> {
  const fullPrompt = `
...
export async function generateNodeFromPrompt(prompt: string): Promise<{ type: NodeType, label: string }> {
    const fullPrompt = `
        You are an expert business process automation assistant.
    ...
    try {
        ...
        if (parsedResponse && typeof parsedResponse === 'object' && 'type' in parsedResponse && 'label' in parsedResponse && availableNodeTypes.includes(parsedResponse.type)) {
            return parsedResponse as { type: NodeType, label: string };
        } else {
